...
Toggl offers two fluent APIs for group membership, inspired by Warden's conductor pattern:
...
// 4. Check feature for user - automatically active via group membership!
...
3. When checking if feature is active for user, Toggl checks:
   ...
   - If active for `__all__`, user inherits it through group membership
...
// 4. Check if user sees beta features (via group membership)
...
5. **Choose between per-user flags vs group membership**
   ```php
   // ✅ Per-user flags for individual access (subscriptions, permissions)
   Toggl::for($user)->activate('premium-features');

   // ✅ Group membership for cohorts (beta testers, experimental rollouts)
