...
final class MigrateCommand extends Command
{
    ...
    private function displayResults(array $stats): void
    {
        ...
        if (array_key_exists('migrations', $stats) && (array_key_exists('migrations', $stats) && $stats['migrations'] !== [])) {
            $this->newLine();
    ...
    private function truncateFeatures(): void
    {
        ...
        assert(is_string($connection), 'Connection must be a string');
        $table = config('toggl.table_names.features', 'features');
        assert(is_string($table), 'Table name must be a string');

        $this->components->warn(sprintf("Truncating table '%s' on connection '%s'...", $table, $connection));
