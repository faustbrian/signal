...
final class Decorator implements CanListStoredFeatures, Driver, HasFlushableCache
{
     ...
     * @param GroupMembershipRepository $membershipRepository   Repository implementation for managing which contexts (users, teams) belong to
    ...
    public function get(string|BackedEnum $feature, mixed $context): mixed
    {
        ...
        // If feature is not directly active (false), check group membership
        // Note: null is treated as an explicitly set value, not "inactive"
        if ($value === false) {
            $value = $this->checkFeatureViaGroupMembership($feature, $context);
        }
    ...
    /**
     * Check if a feature is active via group membership.
     *
     ...
     */
    private function checkFeatureViaGroupMembership(string $feature, TogglContext $context): mixed
    {
