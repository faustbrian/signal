...
final class FeatureManager
{
    ...
    private function resolve(string $name): Decorator
    {
        ...
        return new Decorator(
            ...
            $this->createGroupMembershipRepository($config),
        );
    }
    ...
    /**
     * Create the appropriate group membership repository based on driver configuration.
     *
     * @param  array<string, mixed>      $config The driver configuration
     * @return GroupMembershipRepository The membership repository instance
     */
    private function createGroupMembershipRepository(array $config): GroupMembershipRepository
    {
        $storageDriver = Config::get('toggl.group_storage', $config['driver'] ?? 'array');

        return match ($storageDriver) {
            'database' => $this->container->make(DatabaseGroupMembershipRepository::class),
            default => new ArrayGroupMembershipRepository(),
        };
    }
